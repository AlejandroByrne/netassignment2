    ...
    try:
        ...
        while (True):
            ...
            if (digest_type == 'digest'):
                ...
                # adding table entires to smac_table
                smac_table_entry = p4info_helper.buildTableEntry("MyIngress.smac_table", match_fields={"hdr.ethernet.srcAddr": eth_src_addr})
                

                dmac_table_entry = p4info_helper.buildTableEntry("MyIngress.dmac_forward", match_fields={"hdr.ethernet.dstAddr": eth_src_addr}, action_name="MyIngress.forward_to_port", action_params={"egress_port": port_id})

                smac_table_entry.idle_timeout_ns = int(15 * 1e9) # 15 second timeout 
                dmac_table_entry.idle_timeout_ns = int(15 * 1e9) # 15 second timeout 
